...
public class LoginUserUtil {
    ...
    /*public static UserSuper load() {
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        if (principal != null && principal instanceof UserSuper) {
            return (UserSuper) principal;
        }
        return null;
    }

    public static Long getLoginUserId() {
        UserSuper userSuper = load();
        if (userSuper == null) {
            return -1L;
        }

        return Long.valueOf(userSuper.getId());
    }*/
